...
function submit() {
    ...
    axios.post('/admin/albums/' + props.album.ID + '/image', data)
        .then(() => {
            saved.value = true;
            router.reload();
...
function remove() {
    if (confirm('Are you sure?')) {
        axios.post('/admin/albums/' + props.album.ID + '/image/delete')
            .then(() => {
                router.reload();
            });
    }
...
<template>
    ...
    <form @submit.prevent="submit">
        ...
        <div class="flex gap-8">
            ...
            <div v-if="album.Image" class="text-center">
                <img :src="album.ImagePath" class="object-cover rounded-lg md:w-32 w-20 h-20 md:h-32"/>
                <a href="#"
