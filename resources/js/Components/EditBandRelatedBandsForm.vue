...
let props = defineProps(['genres', 'band', 'bands']);
let selected = ref([]);
let saved = ref(false);
let submitting = ref(false);

onMounted(() => {
    let related_band_ids = props.band.Related.split(',');
    selected.value = props.bands.filter((b) => {
        return related_band_ids.includes('' + b.ID);
    });
});
...
function submit() {
    ...
    let data = {
        bands: selected.value.map((b) => '' + b.ID)
    }
    axios.post('/admin/bands/' + props.band.ID + '/related-bands', data)
        .then(() => {
            saved.value = true;
